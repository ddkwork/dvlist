...
type (
	Data interface {
		Header() []string
		SetColumnWidth(id int, width float32)
		Row(id int) []string
		AddRow(row []string) //AddRow(id int, row []string)  //todo return id
		Len() int
		Sort(id int, ascend bool)
		Filter(columnValue string, id int) //todo return id ,rename FilterColumn FilterRow
	}
	listActType int
	Option      func(list *DVList)
	listAct     struct {
		act listActType
		arg interface{}
	}
...
func WithSelectionHandler(h OnSelectionHandler) Option {
	return func(list *DVList) { list.selectionHandler = h }
}
func WithDoubleClickHandler(h DoubleClickHandler) Option {
	return func(list *DVList) { list.doubleClickHandler = h }
}

func NewDVList(data Data, options ...Option) (*DVList, error) {
	if len(data.Header()) > MaxColumns {
		mylog.Warning("data has more fields (%d) than allowed (%d)", len(data.Header()), MaxColumns)
		return nil, errors.New("len(data.Header()) > MaxColumns ")
	}
	list := &DVList{
		data:           data,
...
func newDVListRender(list *DVList) *dvListRender {
	...
	dlr.header = NewListHeaderRow(list.data.Header(),
...
func (dlr *dvListRender) loadData(layoutsize fyne.Size) {
	...
	for i := startPoint; i < dlr.list.data.Len(); i++ {
		row := NewListBodyRow(i, dlr.list.data.Row(i), dlr.list.onSelectRow,
...
func (dlr *dvListRender) layout(layoutsize fyne.Size, mode int) {
	...
	if mode == listLayoutRedoALL {
		dlr.header = NewListHeaderRow(dlr.list.data.Header(),
